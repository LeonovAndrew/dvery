						...
						foreach ($arYandexFields as $key)
						{
							switch ($key)
							{
								case 'name':
									...
									break;
								case 'description':
									$itemsContent .= "<description>Двери от фабрики. От 3 недель. Индивидуальный дизайн. Любой размер.</description>\n";
									break;
								case 'param':
									if ($parametricFieldsExist)
									{
...
#if($ars = $res->GetNext()) $itemsContent .= '<model>' .$ars['NAME']. "</model>\n";
$itemsContent .= "<vendor>Фабрика Provance</vendor>\n";
$itemsContent .= "<typePrefix>межкомнатные двери</typePrefix>\n";
$itemsContent .= "<sales_notes>Двери в эмали от 26 000 руб. за комплект. От 17 рабочих дней!</sales_notes>\n";
