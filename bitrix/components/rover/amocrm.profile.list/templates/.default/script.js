    ...
    return {
        ...
        getPopup: function (type)
        {
            ...
            if (Object.keys(this.connections).length && Object.keys(this.sourcesList[type]).length)
            {
                buttons.push(
                    new BX.PopupWindowButton({
                        text: BX.message['rover-acrm__button_add'] ,
                        className: "popup-window-button-accept" ,
                        events: {click: function(){

                                var profileSelect = BX('profile'),
                                    connectionSelect = BX('connection');

                                if ((profileSelect.value > 0) && (connectionSelect.value > 0)){
                                    window.location = 'rover-acrm__profile-element.php?source_type=' + type + '&source_id=' + profileSelect.value + '&connection_id=' + connectionSelect.value +'&lang=' + BX.message['rover-acrm__language_id'] ;
                                }
            ...
            buttons.push(new BX.PopupWindowButton({
                text: BX.message['rover-acrm__button_close'] ,
            ...
            content.append(this.getSelect('profile', this.sourcesList[type], BX.message['rover-acrm__' + type + '_select'], BX.message['rover-acrm__' + type + '_empty']));
            content.append(this.getSelect('connection', this.connections, BX.message['rover-acrm__connection_select'], BX.message['rover-acrm__connection_empty']));
                ...
                null,
                {
                    ...
                    titleBar: {
                        content: BX.create("span", {
                            html: '<h2>' + BX.message['rover-acrm__title'] +'</h2>',
