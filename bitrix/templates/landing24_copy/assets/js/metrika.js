...
jQuery('#amoforms_form').on('submit', event => {
	event.preventDefault();
	var name = jQuery("#txtlikeplus").val();
	var phone = jQuery("#txtlikeplus").val();
	var email = jQuery("#txtlikeplus").val();
	var message = jQuery("#txtlikeplus").val();
...
$(document).on('click', '#button_submit', function(e) {
	...
	jQuery.ajax({
	  type: 'post',
	  url: 'https://dveri-provance.ru/lp/contact.php',
	  data: $form.serialize()
	}).done(function() {
		$('.amoforms__fields__submit').hide();
		$('.amoforms__fields__submit__success').show();
		jQuery(document).on('yacounter64922065inited', function () {
		ym(64922065,'reachGoal','nestand_formsubmit');
		});
		ym(64922065,'reachGoal','nestand_formsubmit');
		...
		console.log('Отправка формы успешна');
	}).fail(function() {
	  console.log('fail');
	});
...
$(document).on('click', '#button_messenger_submit', function(e) {
	...
	jQuery.ajax({
	  ...
	  data: $form.serialize()
		...
		window.location.href = ('https://wa.me/79671098956?text=');
	}).fail(function() {
	  console.log('fail');
	});
	//отмена действия по умолчанию для кнопки submit
