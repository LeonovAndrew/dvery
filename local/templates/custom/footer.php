...
$isArticleOrNews = strpos($currentUrl, '/articles/') !== false || strpos($currentUrl, '/news/') !== false;
...
<div class="popup" data-popup="order">
    <form class="popup__cont js-form-ajax-order goal-ym-detail">
        ...
        <div class="popup__text">
            Нажимая кнопку «Получить расчет», Вы принимаете условия обработки Ваших персональных данных
        </div>
        <button type="submit" class="form_order popup__btn">Получить расчет</button>
        <p class="form__message"></p>
    </form>
</div>

<!-- by one cl -->
<div class="popup" data-popup="by_one_cl">
    <form class="popup__cont js-form-ajax-order goal-ym-detail">
        <div class="popup__close">
            <div class="popup__close-item"></div>
            <div class="popup__close-item"></div>
...
<script type="text/javascript">
    jQuery(document).on("mousedown touchstart", 'form button', function() {
		...
		if(location.href.indexOf('/catalog')!=(-1) && $(window).width()<600){ $('.amo-button').remove(); }
