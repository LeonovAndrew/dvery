...
$(function(){
	...
	application.commit.basket.deliveryPeriod = function(deliveryId){
		...
		//clear
		$deliveryPeriodDescription.html("");
	...
	//ajax functions
	application.ajax.sendData = function(url, data, type, dataType, success, forEngageloader = false, contentType = false){
		...
		$.ajax({
			...
			complete: function(jqXHR, textStatus){
				...
				/*var basketPrice = 0;
				var items = jqXHR.responseJSON.compilation.items;

				console.log(jqXHR.responseJSON);

				for (const [key, value] of Object.entries(items)) {
					basketPrice += value.PRICE*value.QUANTITY;
				}

				var basketPricePrint = basketPrice.toString().replace(/(\d)(?=(\d\d\d)+([^\d]|$))/g, '$1 ') + ' р.';
				$('.basketSum').html(basketPricePrint);*/
			},
