...
class SaleApi
{
    ...
    public static function changeBasketQuantity(Api $context, Main\Type\ParameterDictionary $params)
    {
        Main\Loader::includeModule('sale');
        ...
        if ($result->isSuccess()) {
            $result->setData(['id' => $basketId, 'quantity' => $basketItem->getQuantity()]);
        }
    ...
    public static function deleteUserProfile($id)
    {
        ...
        $arUserProps = Sale\Internals\UserPropsTable::getList([
            'filter' => [
                '=ID' => $id,
                '=USER_ID' => $USER->GetID(),
            ],
        ...
        if (!$arUserProps) {
            return ['error' => 'Профайл не найден'];
        }

        $iterator = Sale\Internals\UserPropsValueTable::getList([
            'filter' => [
                '=USER_PROPS_ID' => $id,
            ]
        ]);
        ...
        while ($ar = $iterator->fetch()) {
            Sale\Internals\UserPropsValueTable::delete(($ar['ID']));
        }
    ...
    public static function addUserProfile(Api $context, Main\Type\ParameterDictionary $params)
    {
        global $USER;
        ...
        $personTypeId = Assert::expectIntegerPositive($params['PERSON_TYPE_ID'], 'PERSON_TYPE_ID');

        $personType = Sale\Internals\PersonTypeSiteTable::getList([
            'filter' => [
        ...
        if (!$personType)
            return ['error' => 'Тип профайла не найден'];

        $iterator = Sale\Internals\OrderPropsTable::getList([
            'filter' => [
                '=PERSON_TYPE_ID' => $personType->getPersonTypeId(),
                '=ACTIVE' => 'Y',
            ],
        ]);
        ...
        while ($orderProp = $iterator->fetchObject()) {
            if (!$params->offsetExists('ORDER_PROP_' . $orderProp->getId()))
                continue;

            $value = trim($params['ORDER_PROP_' . $orderProp->getId()]);

            if ($orderProp->getRequired() && empty($value)) {
                return ['error' => 'Свойство "' . $orderProp->getName() . '" не заполнено'];
            }

            if ($orderProp->getIsProfileName()) {
                if (empty($value))
                    return ['error' => 'Свойство "' . $orderProp->getName() . '" не заполнено'];
            ...
            $propsValues[$orderProp->getId()] = $value;
            $properties[$orderProp->getId()] = $orderProp;
        }

        if (!$profileName) {
            return ['error' => 'Не заполнено название профиля'];
        }

        $result = Sale\Internals\UserPropsTable::add([
            'NAME' => $profileName,
            'USER_ID' => $USER->GetID(),
            'PERSON_TYPE_ID' => $personType->getPersonTypeId(),
        ]);
        ...
        $newProfileId = $result->getId();

        foreach ($propsValues as $id => $value) {
            Sale\Internals\UserPropsValueTable::add([
                'USER_PROPS_ID' => $newProfileId,
                'ORDER_PROPS_ID' => $id,
                'NAME' => $properties[$id]->getName(),
                'VALUE' => $value,
            ]);
        }

        return ['success' => true, 'id' => $newProfileId];
    }
    ...
    public static function registerApi(Api $api)
    {
        ...
        $api->registerApi('changeBasketQuantity', array(__CLASS__, 'changeBasketQuantity'));
        ...
        $api->registerApi('deleteUserProfile', array(__CLASS__, 'deleteUserProfile'));
        $api->registerApi('addUserProfile', array(__CLASS__, 'addUserProfile'));
    }
