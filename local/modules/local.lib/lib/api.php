...
class Api {
    ...
    public function processAction()
    {
        ...
        if (!defined('SITE_ID') && $postList->offsetExists('siteId'))
        {
            $siteId = $postList->get('siteId');
            if ($siteId !== '' && preg_match('/^[a-z0-9_]{2}$/i', $siteId))
            {
                define('SITE_ID', $siteId);
            }
        }

        if (!defined('SITE_TEMPLATE_ID') && $postList->offsetExists('templateId'))
        {
            $templateId = $postList->get('templateId');
            if ($templateId !== '' && preg_match('/^[a-z0-9_]+$/i', $templateId))
            {
                define('SITE_TEMPLATE_ID', $templateId);
            }
        }
    ...
    public static function OnPageStart()
    {
        ...
        \Local\Lib\Internals\FormsApi::registerApi(\Local\Lib\Api::getInstance());
        \Local\Lib\Internals\FavoriteApi::registerApi(\Local\Lib\Api::getInstance());
